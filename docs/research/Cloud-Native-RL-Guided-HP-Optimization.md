...
Vivek Kashyap's autoresearch-rl uses an LLM coding assistant as the outer loop, editing a TOML config and running experiments on local GPUs via prime-rl. This approach leverages the LLM's domain knowledge for intelligent search but is coupled to local hardware and non-reproducible.
...
### 3.1 vs. Vivek's autoresearch-rl

| Dimension | Vivek (LLM-as-agent) | Ours (programmatic + cloud) |
|-----------|---------------------|---------------------------|
